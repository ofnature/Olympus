...
public static class PLDActions
{
    ...
    public static class StatusIds
    {
        ...
        public const uint SwordOath = 1902; // Dawntrail: Atonement Ready (was 3-stack Sword Oath pre-7.0)
        public const uint AtonementReady = 1902;
    ...
    {
        if (ActionAvailability.MeetsLevelAndLearned(level, actionService, RoyalAuthority))
            return RoyalAuthority;
        if (ActionAvailability.MeetsLevelAndLearned(level, actionService, RageOfHalone))
            return RageOfHalone;
        return RiotBlade;
    }
    ...
    /// </summary>
    public static ActionDefinition GetSheltronAction(byte level, IActionService? actionService = null)
        => ActionAvailability.Pick(level, actionService, HolySheltron, Sheltron);

    /// <summary>
    /// Gets the best defensive oGCD for the player's level.
    /// </summary>
    public static ActionDefinition GetSentinelAction(byte level, IActionService? actionService = null)
        => ActionAvailability.Pick(level, actionService, Guardian, Sentinel);
