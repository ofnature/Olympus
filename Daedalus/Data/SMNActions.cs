...
public static class SMNActions
{
    ...
    /// </summary>
    public static ActionDefinition GetRuinSpell(byte level, IActionService? actionService = null)
        => ActionAvailability.Pick(level, actionService, Ruin3, Ruin);

    /// <summary>
    /// Gets the best AoE spell for the player's level.
    /// </summary>
    public static ActionDefinition GetAoeSpell(byte level, IActionService? actionService = null)
        => ActionAvailability.FirstAvailable(level, actionService, [TriDisaster, Outburst], Ruin);

    /// <summary>
    /// Gets the Aetherflow spender for single target.
    /// </summary>
    public static ActionDefinition GetAetherflowSpenderST(byte level, IActionService? actionService = null)
        => ActionAvailability.Pick(level, actionService, Necrotize, Fester);

    /// <summary>
    /// Gets the Aetherflow spender for AoE.
    /// </summary>
    public static ActionDefinition GetAetherflowSpenderAoe(byte level, IActionService? actionService = null)
    {
        if (ActionAvailability.MeetsLevelAndLearned(level, actionService, Painflare))
            return Painflare;
        return GetAetherflowSpenderST(level, actionService);
    }

    /// <summary>
    /// Gets the Energy Drain variant based on AoE situation.
    /// </summary>
    public static ActionDefinition GetEnergyDrain(byte level, bool useAoe, IActionService? actionService = null)
    {
        if (useAoe && ActionAvailability.MeetsLevelAndLearned(level, actionService, EnergySiphon))
            return EnergySiphon;
        return EnergyDrain;
    }
